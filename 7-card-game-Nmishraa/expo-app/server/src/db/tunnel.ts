...
export function createSshTunnel(): Promise<{ server: net.Server | null; sshClient: Client }> {
  return new Promise((resolve, reject) => {
    ...
    const localTunnelPort = parseInt(process.env.TUNNEL_PORT || '54334', 10);
    ...
    sshClient.on('ready', () => {
      ...
      server.listen(localTunnelPort, '127.0.0.1', () => {
        console.log(`[SSH Tunnel] Listening on 127.0.0.1:${localTunnelPort} -> forwarding to remote DB server ${sshHost}:${remoteDbPort}`);
